...
### <div style="text-align: center; font-size: 32px; background-color: #242424; color: white; padding: 14px; line-height: 1;border-radius:20px">Pytania</div>
...
1. _Python Enhancement Proposals_
...
3. To dokument opisujący standardy dotyczące stylu kodowania w języku Python
...
1. Jest to zmienna środowiskowa określająca zestaw katalogów, w których znajdują się programy wykonywalne
...
1. Jest to program odpowiedzialny za wykonywanie kodu źródłowego napisanego w języku Python
...
4. Jest to stanowisko w firmie, w ramach którego tłumaczy się kod źródłowy na język biznesowy
...
1. `for (key, value) in some_dict.items()`
...
1. Jest to skrót od tak zwanego uniwersalnego czasu koordynowanego i odnosi się do czasu na długości geograficznej zero stopni
...
1. `output_date = datetime_obj.strftime("%d.%m.%Y")`
...
1. `lambda nums: sum([num**2 for num in nums])`
...
4. To koncepcja programowania, w której funkcja wywołuje samą siebie bezpośrednio lub pośrednio w czasie swojego działania.
...
1. To funkcja, która przyjmuje inną funkcję jako argument i dodaje do niej funkcjonalność lub modyfikuje jej zachowanie, bez konieczności zmiany samej funkcji.
...
2. `print(f"The increase was {number:.0%}")`
...
4. `re.search('Marcin', sentence).span()`
...
Jaką funkcję należy wstawić w miejsce `[???]`, aby po wykonaniu kodu w konsoli zostało wyświetlone zdanie
...
4. Iterator w języku Python to obiekt, który umożliwia iterację po elementach kolekcji (np. listy, krotki) lub sekwencji danych.
...
2. Generator w języku Python to funkcja zawierająca przynajmniej jedno wyrażenie `yield`, które służy do generowania kolejnych wartości w trakcie iteracji.
...
2. `0 Mikiciuk 1 Kowalski 2 Raczkowska 3 Lisicka `
...
4. `new_list = list(map(lambda x: 1 if x >= 3 else 0, numbers))`
...
4. `grouped_names = groupby(names, key=lambda x: x[0])`
...
4. Obsługa wyjątków w Pythonie pozwala na obsługę błędów i wyjątków, które występują podczas wykonywania programu, zapobiegając nagłemu zakończeniu programu.
...
2. `with open('NewScript.txt', 'w') as file:`
...
1. To pakiet wchodzący w skład standardowej biblioteki języka Python służący do tworzenia graficznego interfejsu użytkownika
...
1. Zestaw pisemnych materiałów i komentarzy, które dostarczają informacji o programie, jego modułach, klasach i funkcjach
...
3. Proces analizy, identyfikacji i naprawy błędów (tzw. _bugów_) w kodzie źródłowym programu w celu zapewnienia poprawnego działania aplikacji
...
4. Jest to mechanizm automatycznego zarządzania pamięcią w języku Python, który śledzi i usuwa nieużywane obiekty z pamięci w celu zwolnienia zasobów i uniknięcia wycieków pamięci
